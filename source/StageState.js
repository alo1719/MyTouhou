...
StageState.prototype._initFighter = function () {
    ...
    this.fighterOptionManager = new FighterOptionManager(this, this.fighterManager.elements);
...
//  surface.fillRect( 50, 15, 380 * 1, 5 ) ;
    ...
    if (this.spellCard) {
        ...
        surface.globalAlpha = 0.8;
        surface.fillText('符卡: ' + this.spellCard, 420, 40);
    }
...
StageState.prototype._displayStageTitle = function (surface) {
    ...
    surface.font = '16px Dengxian';
    surface.fillText('关卡' + (this.stageIndex + 1), 100, 210);
    surface.textAlign = 'right';
...
StageState.prototype._drawSide = function (surface) {
    ...
    surface.fillText('弹幕魔法使', this.getWidth() + 30, 70);

    surface.font = '18px Dengxian';
    surface.textAlign = 'right';
    surface.fillText('总得分:', this.getWidth() + 70, 100);
    surface.fillText(this.viewScore, this.getWidth() + 140, 100);
    surface.fillText('火力值:', this.getWidth() + 70, 120);
    surface.fillText(this.fighter.getPower(), this.getWidth() + 140, 120);
    surface.fillText('擦弹数:', this.getWidth() + 70, 140);
    surface.fillText(this.graze, this.getWidth() + 140, 140);
    surface.fillText('自机数:', this.getWidth() + 70, 160);
    surface.fillText(this.players, this.getWidth() + 140, 160);
    surface.fillText('符卡数:', this.getWidth() + 70, 180);
    surface.fillText(this.bombs, this.getWidth() + 140, 180);

    surface.font = '14px Dengxian';
    surface.fillText('自机弹幕数量:', this.getWidth() + 90, 240);
    surface.fillText(this.bulletManager.getNum(), this.getWidth() + 140, 240);
    surface.fillText('敌方小怪数量:', this.getWidth() + 90, 260);
    surface.fillText(this.enemyManager.getNum(), this.getWidth() + 140, 260);
    surface.fillText('敌方弹幕数量:', this.getWidth() + 90, 280);
    surface.fillText(this.enemyBulletManager.getNum(), this.getWidth() + 140, 280);
    surface.fillText('掉落道具数量:', this.getWidth() + 90, 300);
    surface.fillText(this.itemManager.getNum(), this.getWidth() + 140, 300);
...
StageState.prototype.notifyBossVanished = function (boss) {
    this.enemyBulletManager.beItem(this.fighterManager.getClosestFighter(boss));
    this.setFlag(this._FLAG_SE_ENEMY_VANISH);
    this.spellCard = null;
...
KeyFlagQueue.prototype.hasValueOfCount = function (count) {
    while (true) {
        if (this.getLength() === 0)
            return false;
        if (this.min === count)
            return (this.values[0] !== this._NONE);
        if (this.min > count)
            return false;
        this.shift();
    }

...
ElementManager.prototype._initFactory = function () {
    this.factory = new ElementFactory(this.gameStage, this.gameState.maxX, this.gameState.maxY);
};
...
ElementManager.prototype.checkCollisionWith = function (id, element, caller, flag) {
    for (var i = 0, len = this.elements.length; i < len; i++) {
...
ElementManager.prototype.checkCollisionWith2 = function (id, element, caller, flag) {
    for (var i = 0, len = this.elements.length; i < len; i++) {
...
ElementView.prototype.translate = function () {
    for (var j = 0, len = this.getNum(); j < len; j++) {
        ...
        for (var i = 0; i < this._V_ITEM_NUM; i++) {
            this.vertices[o + i * this._V_ITEM_SIZE] += this._getElementX();
...
ElementView.prototype.rotate = function () {
    ...
    for (var j = 0, len = this.getNum(); j < len; j++) {
        var o = this._V_SIZE * j;
        for (var i = 0; i < this._V_ITEM_NUM; i++) {
            var x = this.vertices[o + i * this._V_ITEM_SIZE];
            var y = this.vertices[o + i * this._V_ITEM_SIZE + 1];

            this.vertices[o + i * this._V_ITEM_SIZE] =
...
function Element(gameState, maxX, maxY) {
    this.moveVectorManager = __moveVectorManager;
    this.listId = null;
    this.listForw = null;
    this.image = null;
...
Element.prototype.inCollisionArea = function (x, y) {
    return x >= this.getCollisionLeftX() && x <= this.getCollisionRightX() &&
...
Element.prototype.checkCollision = function (e) {
    ...
    return (this.inCollisionArea(e.getCollisionLeftX(), e.getCollisionUpY()) ||
...
Element.prototype.inGrazeArea = function (x, y) {
    return x >= this.getGrazeLeftX() && x <= this.getGrazeRightX() &&
...
Element.prototype.checkGraze = function (e) {
    ...
    return (this.inGrazeArea(e.getGrazeLeftX(), e.getGrazeUpY()) ||
...
Element.prototype._outOfTheField = function () {
    return this.getX() < 0 || this.getX() > this.maxX || this.getY() < 0 || this.getY() > this.maxY;
